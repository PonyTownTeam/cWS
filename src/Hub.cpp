...
void Hub::onServerAccept(cS::Socket *s) {
    ...
    httpSocket->start(httpSocket->nodeData->loop, httpSocket, httpSocket->setPoll(EVENT_READABLE));
...
void Hub::onClientConnection(cS::Socket *s, bool error) {
    ...
    if (error) {
        ...
        httpSocket->change(httpSocket->nodeData->loop, httpSocket, httpSocket->setPoll(EVENT_READABLE));
        httpSocket->setNoDelay(true);
...
void Hub::upgrade(uv_os_sock_t fd, const char *secKey, const char *extensions, size_t extensionsLength, const char *subprotocol, size_t subprotocolLength, Group<SERVER> *serverGroup) {
    ...
    webSocket->change(webSocket->nodeData->loop, webSocket, webSocket->setPoll(EVENT_READABLE));
